...
public class DeleteTicket extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        // Lấy mã giao dịch từ phiên thanh toán
        HttpSession session = request.getSession();
//        String transactionId = (String) session.getAttribute("transactionId");
        String paymentId = request.getParameter("paymentId");

        if (paymentId!=null) {
            // Sử dụng mã giao dịch để tạo phiên hoàn tiền
        RefundCreateParams refundParams = RefundCreateParams.builder()
                .setPaymentIntent(paymentId) // Sử dụng mã giao dịch làm ID phiên thanh toán
                .build();
        ...
        try {
            ...
            session.setAttribute("error", "Mã giao dịch không chính xác");
            response.sendRedirect("historyBookingController");
            return;
        }
        }
