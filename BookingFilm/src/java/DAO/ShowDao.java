...
// lấy film id với title đẻ show ra trang chủ
...
// tạo xuất chiếu
    ...
    // lấy các suất chiếu film cụ thể - kiểm tra ngày này có show nào không
...
System.out.println("getShow error");
            return null;
        }
    }
...
//        List<FilmSlide> film = db.getAllFilmShow();
        List<Show> show = db.getShow(1, Date.valueOf("2023-10-24"));
...
//        Date dateNow = Date.valueOf(now);
        List<Date> date = new ArrayList<>();
         for (int i = 0; i < 7; i++) {
            ...
            List<Show> show_test = db.getShow(1, sqlDate); // kiẻm tra ngày đó có show nào không
            if(!(show_test.isEmpty())){date.add(sqlDate);} // có show thì mới add vào list date
